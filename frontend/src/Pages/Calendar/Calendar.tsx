...
import { fetchCards, fetchLists, getMemberships } from "../../Utils/apiServices";
              ...
              <ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
                {selectedDayMeetings.length > 0 ? (
                  selectedDayMeetings.map((card) => (
                    <div key={card.title}>
                      <p className="text-slate-300 text-base font-semibold">{card.list.board.title} - {card.list.title}</p>
                      <p className="text-slate-300 text-sm">{card.title}</p>
                      <p className="text-slate-300 text-sm">{card.description}</p>
                      <p className="text-slate-300 text-sm">{formatPostDate(card.due_date)}</p>
                    </div>
                  ))
                ) : (
